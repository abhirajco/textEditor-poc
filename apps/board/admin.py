...
class TaskAdmin(admin.ModelAdmin):
    # These names must exactly match the field names in your Task model
    list_display = (
        "title", 
        "status", 
        "priority", 
        "campaign",  # Your ForeignKey field name
        "event",     # Your ForeignKey field name
        "assigned_to", 
        "due_date"
    )
    
    # Now that these are ForeignKeys, list_filter will 
    # automatically create a drill-down sidebar for them.
    list_filter = ("status", "priority", "campaign_id", "event_id")
    
    search_fields = ("title", "assigned_to__username", "assigned_to__email")
    
    # Optional: Improves performance for large datasets by 
    # using a lookup instead of a massive dropdown.
    raw_id_fields = ("campaign", "event")
